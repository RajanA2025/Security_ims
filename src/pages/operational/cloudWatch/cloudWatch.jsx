...
const Business = () => {
  ...
  // 🔹 Safe username extractor (used in rowKey fallback and rendering)
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        try {
          ...
          // localStorage might be unavailable in some test environments — swallow safely
          devError("localStorage.getItem failed:", err);
          raw = null;
        }
        ...
        if (raw == null) {
          ...
          // Try to parse JSON only if it looks like JSON (starts with [ or { or a quoted string ' or ")
          ...
          if (trimmedRaw === "") {
            ...
            try {
              storedIds = JSON.parse(trimmedRaw);
              // If parsing yields null or non-array/non-string, coerce below
            } catch {
              // if parse fails, fallback to splitting by commas (common non-JSON format)
              storedIds = trimmedRaw.split(",").map((s) => s.trim());
            }
          } else {
